...
class anglano2014_fc2q_mimo_v2_application_manager: public base_application_manager<TraitsT>
{
	...
	private: void init()
	{
		...
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaC is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaC is BDW", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + cres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaC is BDW", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is BUP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is LOW and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is FINE and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is UP", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is NEG and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is STY", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is OK and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is NEG then DeltaM is DWN", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is ZERO then DeltaM is BDW", p_fuzzy_eng_.get()));
		p_rules->addRule(fl::Rule::parse("if " + mres_fuzzy_var_name + " is HIGH and " + err_fuzzy_var_name + " is POS and " + deltaerr_fuzzy_var_name + " is POS then DeltaM is BDW", p_fuzzy_eng_.get()));
