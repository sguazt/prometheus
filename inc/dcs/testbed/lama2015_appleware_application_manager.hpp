/**
 * \file dcs/testbed/lama2015_appleware_application_manager.hpp
 *
...
#ifndef DCS_TESTBED_LAMA2015_APPLEWARE_APPLICATION_MANAGER_HPP
#define DCS_TESTBED_LAMA2015_APPLEWARE_APPLICATION_MANAGER_HPP
...
class lama2015_appleware_application_manager: public base_application_manager<TraitsT>
{
    ...
    public: lama2015_appleware_application_manager()
    : output_order_(1),
...
#ifdef DCS_DEBUG
{
DCS_DEBUG_STREAM << "OUT_PERF_HISTORY - BEFORE: [";
for (std::size_t k = 0; k < out_perf_history_[cat].size(); ++k)
{
    DCS_DEBUG_STREAM << ", " << out_perf_history_[cat].at(k);
}
...
#ifdef DCS_DEBUG
{
DCS_DEBUG_STREAM << "OUT_PERF_HISTORY - AFTER: [";
for (std::size_t k = 0; k < out_perf_history_[cat].size(); ++k)
{
    DCS_DEBUG_STREAM << ", " << out_perf_history_[cat].at(k);
}
            ...
            if (ok)
            {
                ...
                for (std::size_t i = 0; i < nvms; ++i)
                {
                    ...
                    for (std::size_t j = 0; j < num_vm_perf_cats; ++j)
                    {
                        ...
                        real_type new_share = std::max(std::min(new_xshares[k], 1.0), 0.05);
...
DCS_DEBUG_TRACE("Control applied");//XXX
    ...
    private: void init_anfis()
    {
		...
		DCS_DEBUG_TRACE( p_anfis_eng_->toString() );
    }
...
#ifdef DCS_DEBUG
{
DCS_DEBUG_STREAM << "ANFIS - TRAINING INSTANCE: <";
DCS_DEBUG_STREAM << "IN: [";
for (std::size_t i = 0; i < inputs.size(); ++i)
{
    DCS_DEBUG_STREAM << ", " << inputs[i];
}
DCS_DEBUG_STREAM << "], OUT: [";
for (std::size_t i = 0; i < num_outputs_; ++i)
{
    DCS_DEBUG_STREAM << ", " << y_train[i];
}
DCS_DEBUG_STREAM << "]>" << std::endl;
}
#endif // DCS_DEBUG
//[/XXX]
        ...
        if (anfis_initialized_)
        {
            ...
            real_type rmse = 0;
//[FIXME]: This info is not available in the Lama's paper
...
#ifdef DCS_DEBUG
{
DCS_DEBUG_STREAM << "ANFIS - PROCESS: <";
DCS_DEBUG_STREAM << "IN: [";
for (std::size_t i = 0; i < p_anfis_eng_->numberOfInputVariables(); ++i)
{
    DCS_DEBUG_STREAM << ", " << p_anfis_eng_->getInputVariable(i)->getValue();
}
DCS_DEBUG_STREAM << "], OUT: [";
for (std::size_t i = 0; i < p_anfis_eng_->numberOfOutputVariables(); ++i)
{
    DCS_DEBUG_STREAM << ", " << p_anfis_eng_->getOutputVariable(i)->getValue();
}
DCS_DEBUG_STREAM << "]>" << std::endl;
}
#endif // DCS_DEBUG
//[/XXX]
        ...
        {
            // Build the initial ANFIS model
            // - How much large should be the initial data set?
            // - How many epoch are needed to train the model?
            // NOTE: this step should be perform offline (see Lama's paper).
            //       Indeed, we do this by using a trial-and-error approach based on subtractive clustering and ANFIS model selection
...
//  std::ostringstream oss;
//  oss << "rubis_lama2015appleware_n" << ctrl_count_;
...
DCS_DEBUG_STREAM << "Zeta*(i,:) : " << ublas::row(Zeta_star, i) << std::endl;//XXX
DCS_DEBUG_STREAM << "Eta*(i,:) : " << ublas::row(Eta_star, i) << std::endl;//XXX
DCS_DEBUG_STREAM << "Theta*(i,:) : " << ublas::row(Theta_star, i) << std::endl;//XXX
DCS_DEBUG_STREAM << "OUTPUT*(i) : " << ublas::inner_prod(ublas::row(Zeta_star, i), xi) + ublas::inner_prod(ublas::row(Eta_star, i), u) + Theta_star(i,0) << std::endl;//XXX
#endif // DCS_DEBUG
        }

        const std::size_t nxlin = nxi+1;
#ifdef DCS_DEBUG
DCS_DEBUG_STREAM << "# XLIN: " << nxlin << std::endl;//XXX
#endif // DCS_DEBUG
...
#ifdef DCS_DEBUG
DCS_DEBUG_STREAM << "x_{lin} : " << xlin << std::endl;//XXX
DCS_DEBUG_STREAM << "A : " << A << std::endl;//XXX
DCS_DEBUG_STREAM << "B : " << B << std::endl;//XXX
DCS_DEBUG_STREAM << "C : " << C << std::endl;//XXX
#endif // DCS_DEBUG
...
}; // lama2015_appleware_application_manager

template <typename T>
const std::size_t lama2015_appleware_application_manager<T>::control_warmup_size = 5;

template <typename T>
const float lama2015_appleware_application_manager<T>::resource_share_tol = 1e-2;

}} // Namespace dcs::testbed

#endif // DCS_TESTBED_LAMA2015_APPLEWARE_APPLICATION_MANAGER_HPP
