/**
 * \file dcs/testbed/wang2015_fmpc_application_manager.hpp
 *
 * \brief Application manager based on the FMPC solution proposed in
...
#ifndef DCS_TESTBED_WANG2015_FMPC_APPLICATION_MANAGER_HPP
#define DCS_TESTBED_WANG2015_FMPC_APPLICATION_MANAGER_HPP
...
namespace wang2015_fmpc_detail {
...
//DCS_DEBUG_TRACE("Read from MATLAB --> " << line);//XXX
            ...
            if (parse_line)
            {
                if (line.find("[/dcs::testbed::wang2015_fmpc_application_manager]") != std::string::npos)
                {
...
class wang2015_fmpc_application_manager: public base_application_manager<TraitsT>
{
    ...
    public: wang2015_fmpc_application_manager()
    : output_order_(1),
...
//[/XXX]
        ...
        if (anfis_initialized_)
        {
            ...
            real_type rmse = 0;
//[FIXME]: This info is not available in the Wang's paper
...
//[/XXX]
        ...
        else
        {
            ...
            // NOTE: this step should be perform offline (see Wang's paper).
...
DCS_DEBUG_TRACE("ANFIS TRAINED FIRST TIME -> RMSE: " << rmse);//XXX
    ...
    private: std::vector<real_type> perform_mpc_control()
    {
...
#ifndef DCS_TESTBED_HAVE_MATLAB
        DCS_EXCEPTION_THROW(std::runtime_error, "MATLAB is not available (needed to run genetic algorithms in Wang's FMPC application manager)");
#endif
        ...
        oss << "-r \""
            ...
            << "   gaopts = gaoptimset('TimeLimit', " << (this->control_time()*6) << ");" // We need to transpose x0 since it is represented as a column vector
            << "   LB = zeros(size(x0));"
            << "   UB = ones(size(x0));"
            //<< "   rng(1, 'twister');" // For reproducibility
            << "   [x,fval,exitflag] = ga(objfun, nvar, [], [], [], [], LB, UB, [], gaopts);"
            << "   format long;"
            << "   disp('--- [dcs::testbed::wang2015_fmpc_application_manager] ---');"
            << "   disp(['x=', mat2str(x)]);"
        ...
        wang2015_fmpc_detail::matlab_output_consumer<real_type> matlab_consumer;
...
}; // wang2015_fmpc_application_manager

template <typename T>
const std::size_t wang2015_fmpc_application_manager<T>::control_warmup_size = 5;

template <typename T>
const float wang2015_fmpc_application_manager<T>::resource_share_tol = 1e-2;

}} // Namespace dcs::testbed

#endif // DCS_TESTBED_WANG2015_FMPC_APPLICATION_MANAGER_HPP
