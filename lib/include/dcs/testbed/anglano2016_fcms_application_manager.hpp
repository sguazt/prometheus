/**
 * \file dcs/testbed/anglano2016_fcms_application_manager.hpp
 *
 * \brief Application manager based on the work by (Anglano et al., 2016)
 *
...
#ifndef DCS_TESTBED_ANGLANO2016_FCMS_APPLICATION_MANAGER_HPP
#define DCS_TESTBED_ANGLANO2016_FCMS_APPLICATION_MANAGER_HPP
...
//#include <dcs/math/function/trunc.hpp>
 ...
 * \brief Application manager based on the work by (Anglano et al., 2016)
 *
 * This class implements the FCMS fuzzy controller proposed in [1].
 *
 * References:
 * -# C. Anglano, M. Canonico and M. Guazzone,
 *    "FCMS: a Fuzzy Controller for CPU and Memory Consolidation under SLA Constraints,"
...
class anglano2016_fcms_application_manager: public base_application_manager<TraitsT>
{
	...
	public: anglano2016_fcms_application_manager()
	: beta_(0.9),
        ...
        if (!skip_ctl)
        {
			...
			try
			{
				for (std::size_t i = 0; i < nvms; ++i)
				{
					...
					//new_share = dcs::math::trunc(std::ceil(new_share/resource_share_tol))*resource_share_tol;
...
}; // anglano2016_fcms_application_manager

template <typename T>
const std::size_t anglano2016_fcms_application_manager<T>::control_warmup_size = 5;

template <typename T>
const float anglano2016_fcms_application_manager<T>::resource_share_tol = 1e-2;
//const float anglano2016_fcms_application_manager<T>::resource_share_tol = 1e-3;

template <typename T>
const float anglano2016_fcms_application_manager<T>::resource_share_lb_scale_factor = 1.1;
//const float anglano2016_fcms_application_manager<T>::resource_share_lb_scale_factor = 1;

template <typename T>
const std::string anglano2016_fcms_application_manager<T>::err_fuzzy_var_name = "E";

template <typename T>
const std::string anglano2016_fcms_application_manager<T>::cres_fuzzy_var_name = "Cres";

template <typename T>
const std::string anglano2016_fcms_application_manager<T>::deltac_fuzzy_var_name = "DeltaC";

template <typename T>
const std::string anglano2016_fcms_application_manager<T>::mres_fuzzy_var_name = "Mres";

template <typename T>
const std::string anglano2016_fcms_application_manager<T>::deltam_fuzzy_var_name = "DeltaM";

}} // Namespace dcs::testbed

#endif // DCS_TESTBED_ANGLANO2016_FCMS_APPLICATION_MANAGER_HPP
