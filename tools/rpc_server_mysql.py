...
class NetworkConnectionsManager:
	...
	def __init__(self, db_uri):
		# Register "unxi" and "tcp" schemes to let 'urlparse' to correctly parse associated URIs
		register_uri_scheme('unix')
	...
	def connection_stats(self, host, port):
		sql = ("SELECT num_arrivals,num_departures,last_update"
			   " FROM network_connection_stat"
			   " WHERE server_addr=%s AND server_port=%s")
		...
		try:
			...
			db_cursor.execute(sql, (host, port))
			for (na,nd,ts) in db_cursor:
				#print("THERE.6: {} - {} - {}".format(na, nd, ts))
				num_arr = na
				num_dep = nd
				upd_ts = time.mktime(ts.timetuple())
			db_cursor.close()
		...
		return (num_arr, num_dep, upd_ts)

	def num_pending_connections(self, host, port):
		return self.num_connections_by_status_impl_(host, port, None)

	def num_connections_by_status(self, host, port, status):
		return self.num_connections_by_status_impl_(host, port, status)

	def num_connections_by_status_impl_(self, host, port, status):
		sql = ""
		if status == None:
			sql = ("SELECT num_arrivals-num_departures"
				   " FROM network_connection_stat"
...
class RpcFuncs:
	...
	def connection_stats(self, host="localhost", port=9090):
		"""Return a tuple containing the number of arrived and departed TCP
		   connections at the given host and port, along with the last update
		   timestamp."""
		mgr = NetworkConnectionsManager(self.db_uri_)
		return mgr.connection_stats(host, port)

	def num_pending_connections(self, host="localhost", port=9090):
		"""Return the number of pending and served TCP connections at the given
		   host and port."""
		mgr = NetworkConnectionsManager(self.db_uri_)
	...
	def num_connections_by_status(self, host="localhost", port=9090, status=NetworkConnectionsManager.wait_connection_status):
		"""Return the number of TCP connections to the given host and port and
